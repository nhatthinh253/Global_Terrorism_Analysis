...
# Key takeaways
st.markdown('Up to 2011, the number of attacks never exceeded 5000 per year. However, there has been a major increase in the number of attacks since 2012, with a spike to almost 17k attacks in 2014')

# Import data_1
data1 = load_data('/Users/thinhcn/Desktop/Weekly project/Week 3/data1.csv')
...
# Part 2  Attacks by region from 1970 to 2017
...
# Part 3 Terrorist groups across the period

st.markdown("## " + '**3. Terrorist groups across the period**')
...
# Key takeaways
st.markdown('Before 2000, Farabundo Marti National Liberation Front (FMLN) and Shining Path (SL) were the two major groups.'+
' These groups gradually weakened and seemed to stop in 1990s. Taliban, Islamic State of Iraq and the Levant (ISIL), '+
'and Al-Shabaab emerged in 2000s and have quickly become dominant in their own regions: South Asia, MENA, '+ 
...
st.text(f'Total number of attacks: {map_data.shape[0]}')
st.text(f'Total number of casualties: {int(map_data.ncasualty.sum())}')
...
# Clone data1 and add ncasualty col from cached data
...
st.markdown('- The War in Iraq was an armed conflict. the Iraqi insurgency escalated into a full-scale war with the conquest of Ramadi, Fallujah, Mosul, Tikrit and in the major areas of northern Iraq by the ISIS')

# South Asia
st.markdown("### " + '** South Asia**')
...
# Part 4 Spending vs Casualties
st.markdown("## " + '**4. Military Spending vs. Casualties**')

# load new data
warspending = pd.read_csv(r'/Users/thinhcn/Desktop/Weekly project/Week 3/Militaryspending.csv',encoding='ISO-8859-1')
...
# Target of each terrorist group

st.markdown("## " + '**Appendix: Target of each terrorist group**')
...
selected_group = st.selectbox(
    label="Choose a terrorist group", options= all_groups
)
